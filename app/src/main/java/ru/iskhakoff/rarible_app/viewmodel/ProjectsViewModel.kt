...
class ProjectsViewModel(private val apiRepository: ApiRepository, private val prefsRepository: PrefsRepository) : ViewModel() {
    ...
    fun getProjects() {
        ...
        viewModelScope.launch(handler) {
            val response = apiRepository.getProjects(prefsRepository.getToken())
            when (response.code) {
                200 -> {
